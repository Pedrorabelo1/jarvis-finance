...
export default function FixasPage() {
  ...
  const totalAtivas = ativas.reduce((s, f) => s + f.valor, 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs text-secondary">Total mensal {tab === 'ativas' ? 'ativas' : 'encerradas'}</div>
        ...
        <div className="flex gap-1 p-1 rounded-lg bg-white/5 border border-white/10">
          {(['ativas', 'encerradas'] as const).map((t) => (
            ...
            >
              {t === 'ativas' ? `Ativas (${ativas.length})` : `Encerradas (${encerradas.length})`}
            </button>
          ))}
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-blue-400 text-white text-sm font-medium flex items-center gap-2 shadow-md"
        >
      ...
      {loading ? (
        ...
        <div className="glass-card p-12 text-center text-secondary">Nenhuma despesa nesta aba.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {lista.map((f) => {
            ...
            return (
              ...
              >
                {f.categoria && (
                  ...
                  >
                    <IconRenderer name={f.categoria.icone} className="w-5 h-5" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <div className="font-medium text-primary truncate">{f.descricao}</div>
                    {f.status === 'pausada' && <Badge color="#f59e0b">Pausada</Badge>}
                    {f.status === 'encerrada' && <Badge color="#94a3b8">Encerrada</Badge>}
                  </div>
                  <div className="text-sm text-secondary tabular">
                    <HiddenValue value={f.valor} /> · venc. dia {f.diaVencimento}
                  </div>
                  {f.status === 'ativa' && (
                  ...
                  {f.status === 'encerrada' && f.dataFim && (
                    <div className="text-xs text-tertiary">Encerrada em {formatDate(f.dataFim)}</div>
                  )}
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  {f.status === 'ativa' ? (
                    <button
                      onClick={() => updateStatus(f.id, 'pausada')}
                    ...
                    <button
                      onClick={() => updateStatus(f.id, 'ativa')}
                      className="p-1.5 rounded-lg hover:bg-emerald-500/15 text-emerald-400"
                      title="Reativar"
                    >
                  ...
                  {f.status !== 'encerrada' && (
                    <button
                      onClick={() => updateStatus(f.id, 'encerrada')}
